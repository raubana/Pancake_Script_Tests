	...
	def _process(L):
		...
		while i < len(L):
			...
			if t in (TYPE_NUMBER,TYPE_TERM,TYPE_STRING):
				output.append(token)
			elif t == TYPE_CALL and token.value[1].type == TYPE_FUNCTION:
				stack.append(token)
			elif t == TYPE_SEPARATOR:
				while len(stack) > 0 and stack[-1].type != TYPE_SEPARATOR:
					output.append(stack.pop())
			elif t == TYPE_CALL and token.value[1].type == TYPE_OPERATOR:
				op1 = find_op_by_symbol(token.value[1].value, token.value[0])
				while len(stack) > 0 and stack[-1].type == TYPE_CALL and stack[-1].value[1].type == TYPE_OPERATOR:
					op2 = find_op_by_symbol(stack[-1].value[1].value, stack[-1].value[0])
					print op1, op2, token, stack[-1]
				...
				i = end_index
				if token.value == TUPLE_START_CHAR and len(stack) > 0 and stack[-1].type == TYPE_CALL and stack[-1].value[1].type == TYPE_FUNCTION:
					output.append(stack.pop())
			else:
				print 'uh oh...', token
			i += 1
