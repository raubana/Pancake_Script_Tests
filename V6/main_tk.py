...
class Main(object):
	...
	def highlight_error(self, error):
		self.script_element.tag_config("error", background="red", foreground="black")

		cur_tok = self.compiled_script.tokens[error.pc_line]
		...
		self.place_tag(self.script_element, "error", line_num, char_num, length)
	...
	def do_next(self):
		...
		while self.interpreter is not None and self.interpreter.running and iterations > 0:
			...
			if self.interpreter.running:
				result = self.interpreter.process_current_line()
				if result and result.TYPE == "EXCEPTION":
					self.stop()
					self.highlight_error(result)
					self.output_element.config(state=Tkinter.NORMAL)
					self.output_element.insert(Tkinter.END, repr(result))
					self.output_element.config(state=Tkinter.DISABLED)
				else:
					self.analyze_output()
